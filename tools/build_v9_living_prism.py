#!/usr/bin/env python3
"""Create the V9 Living Prism cooler frames from the square V7 master."""
...
def crystal_wave(phase: float) -> Image.Image:
    ...
    ImageDraw.Draw(crystal_mask).polygon(
        [(239, 48), (289, 129), (314, 244), (292, 352), (243, 379), (186, 319), (181, 191)],
        fill=255,
    )
...
def glint_layer(phase: float) -> Image.Image:
    events = [
        (0.10, (239, 55), (255, 135, 76)),
        (0.36, (290, 148), (255, 244, 188)),
        (0.61, (190, 258), (74, 224, 255)),
        (0.84, (279, 326), (255, 73, 229)),
    ]
...
def floor_layer(phase: float) -> Image.Image:
    ...
    for ring in range(5):
        ...
        box = (240 - radius, 412 - vertical, 240 + radius, 412 + vertical)
    ...
    for index, color in enumerate(((0, 213, 255), (255, 36, 210), (86, 72, 255))):
        ...
        for x in range(-20, 501, 8):
            y = 423 + index * 13 + 5 * math.sin(math.tau * (x / 240 + phase * (1 + index * 0.25)))
            points.append((x, y))
...
def internal_glow_layer(phase: float) -> Image.Image:
    ...
    positions = [
        ((197, 98, 268, 196), (255, 76, 42), int(15 + 10 * pulse)),
        ((205, 175, 295, 286), (0, 220, 255), int(14 + 12 * (1 - pulse))),
        ((190, 270, 292, 372), (197, 42, 255), int(13 + 10 * pulse)),
    ]
...
def render_frame(base: Image.Image, phase: float, particles) -> Image.Image:
    ...
    frame = ImageEnhance.Color(frame).enhance(1.035 + 0.035 * pulse)
    frame = ImageEnhance.Contrast(frame).enhance(1.012)
